...
8a. 【美股板块行情来源与首页双排行（强制）】生成正文前必须分别抓取 Moomoo 美股行业排行和 Futunn/Moomoo Sparks 概念排行，并在首页左侧日历下方的 `US_SECTOR_PANEL` 与 `US_SECTOR_PANEL_EN` 各自既有 marker 内生成上下排列的两个独立排行榜分组：上方“隔夜美股行业排行”，下方“隔夜美股概念排行”。两套排行不得混排、合并或仅凭同名板块互相替代；同名板块必须按各自 BK 编号和成分口径独立展示。本条统一规定板块行情来源、双排行展示及校验；发布传输统一执行本文件“发布传输唯一流程”。
   - 行业排行主来源固定为 `https://www.moomoo.com/hans/quote/us/sector-industry`。必须完整读取全部分页中的正负行业板块，记录行业名称、涨跌幅、领涨/代表股及其涨跌幅、涨/跌/平家数，重点检查黄金、贵金属、白银、铜、半导体等重大映射方向。不得用个股代理样本冒充已完成的行业榜扫描；若页面因域名权限、超时或反爬无法完整取得，行业分组必须明确写“行业板块榜未完成”，不得把代理样本表述为完整板块行情，并必须用可验证公开行情交叉核验黄金、贵金属等重大遗漏风险。
   - 概念排行使用 Sparks，首选抓取 `https://www.futunn.com/quote/sparks-us?chain_id=2keKWYi3XB0JW1.1l4cq9k&global_content=%7B%22promote_id%22%3A13766,%22sub_promote_id%22%3A41,%22f%22%3A%22nn%2Fquote%2Fcalendar%22,%22b%22%3A%22Tab_%E4%B8%89%E7%BA%A7_Markets-Trading%20Tools-Investment%20Themes%22%7D`，使用桌面浏览器 UA；若首选 URL 失败，再回退抓取 `https://www.moomoo.com/hans/quote/sparks-us`。优先直接解析 HTML 中 `window.__INITIAL_STATE__.sparks_index.list`，解析一次后缓存并供 panel 与正文共同复用；不要为了同一概念数据再重复抓取 `/quote/us/concepts`，也不要依赖页面截图或手工浏览。
   - 行业分组和概念分组各自必须输出“正数 Top5 + 倒数 Top5”共 10 行：先按板块涨跌幅从高到低取正收益前 5 个，再按涨跌幅从高到低排列负收益倒数 5 个（轻跌在上、跌最多在最下面）。每行必须包含板块名、板块涨跌幅、领涨/代表股票代码及该股涨跌幅、板块内涨/跌/平家数。概念分组优先使用 Sparks 的 `plateName/changeRatio/stockCode/stockChangeRatio/priceRiseCount/priceFallCount/priceSameCount`；可靠负收益行不足时可用已核验压力样本补足，但必须在该分组的 `spark-source` 披露，不能把补足样本表述为完整概念榜。
   ...
   - 发布前必须机器校验：行业分组在前、概念分组在后；数据成功取得时，每个分组恰好 10 个 `spark-row` 和 5 个 `spark-change down`，各自倒数 5 行按跌幅从小到大排列，整个 marker 合计恰好 20 行和 10 个 down；任一分组抓取失败时，该分组可以不输出排行行，但必须保留中英文标题和明确失败说明，且另一成功分组仍须单独满足 10 行/5 down。英文标题固定为 `Overnight US Industry Ranking` 和 `Overnight US Concept Ranking`，顺序、行数、ticker、涨跌幅、涨跌家数和时间点与中文逐项一致。英文标题时间不得硬编码显示 CST，必须复用页面 `toNewYorkTime()`/`localizeNewYorkTimes()` 转换为 `America/New_York` 短时区；中文仍显示 CST。
